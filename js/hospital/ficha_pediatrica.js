...
function guardar_consulta() {
  if ($("#motivo_consulta").val() == "") {
    ...
    if (estado == 0) {
      $.ajax({
        url: 'ficha_pediatrica/guardar_ficha',
        type: "POST",
        dataType: "json",
        data: datos,
        beforeSend: function() {
      ...
      $.ajax({
        url: 'ficha_pediatrica/actualizar_consulta',
        type: "POST",
        data: datos,
        beforeSend: function() {
          $('#myModal').modal('toggle');
...
function guardar_antecedentes() {
  var datos = $("#form_antecedentes_pediatricos").serialize();
  $.ajax({
    url: 'ficha_pediatrica/guardar_antecedentes',
    type: "POST",
    dataType: "json",
    data: datos,
    beforeSend: function() {
    ...
    success: function(response) {
      if (response == 0) {
        ...
        swal({
          title: 'Oops...',
          text: 'Hubo un error al guardar los antecedentes, por favor intentelo de nuevo mas tarde!',
          type: 'error',
          confirmButtonColor: '#4fb7fe'
        }).done();
      }
