...
function seach_company_venta($datos){
  $CI =& get_instance();
      ...
      <?php $i = 1;  foreach($query->result_array() as $row){  ?>
        <tr>
          ...
          <td style="font-size: 12px;" class="text-center">
			...
			<form action="Consumidor_final" method="POST">
				<input type="hidden" id="id" name="id" value="<?=$row['Idempresa'];?>">
				<input class="btn btn-info" type="submit" value="CON. FINAL">
			</form>
			<form action="Credito_fiscal_venta" method="POST">
				<input type="hidden" id="id" name="id" value="<?=$row['Idempresa'];?>">
				<input class="btn btn-success" type="submit" value="CREDITO FISCAL">
			</form>
            </div>
          </td>
        </tr>
