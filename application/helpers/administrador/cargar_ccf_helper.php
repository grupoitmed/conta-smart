<?php
 ...
 function load_facturasccf($array){
    $CI =& get_instance();
    $CI->load->helper('url');
    $CI->load->database();
	...
	$ahora = date('Y-m-d');
	extract($array);
	if($fhasta != '' && $finicio != ''){ $consulta = 'AND fecha BETWEEN "'.$finicio.'" AND "'.$fhasta.'"'; }
	else{ $consulta = 'AND DATE(fecha_creacion) = "'.$ahora.'"';}
...
<table class="table table-striped" id="archivos_table"s>
    ...
    <tr>
      ...
      <th style="text-align:center;" style="font-size:12px;" width="">NRC</th>
	...
	WHERE estado != 3 ".$consulta." AND tipo_fac = 1 AND idempresa = ".$id."");
	foreach($sql->result_array() as $row){
		...
		<td style="font-size:12px;" align="center"><?php echo $row['nrc']?></td>
		...
		<td style="font-size:12px;" align="center">
			<button class="btn btn-primary" onclick="editar_cfiscal(<?php echo $row['idFactura']?>);"><i class="fa fa-refresh"></i></button>
			<button class="btn btn-danger" onclick="eliminar_facturaccf(<?php echo $row['idFactura']?>)"><i class="fa fa-trash"></i></button>
			<button class="btn btn-warning" onclick="anular_facturaccf(<?php echo $row['idFactura']?>)"><i class="fa fa-times-circle"></i></button>
		</td>
		</tr>
		<?php
	}
